  ...
  return (
    <div className="mt-3 min-w-0 space-y-3">
      ...
      <div className="flex min-w-0 flex-wrap gap-2">
        <CapturedContentViewer buttonLabel="explainability.actions.viewDriftState" title={t("explainability.labels.driftState")} content={summary.exactStateContext} testId="exact-drift-state" exactTabLabel="explainability.labels.exactSource" copyLabel="explainability.actions.copyExactDriftState" description="explainability.messages.driftStatePreviewNotice" />
        <CapturedContentViewer buttonLabel="explainability.actions.viewReduceContext" title={t("explainability.labels.reduceContext")} content={summary.exactReduceContext} testId="exact-drift-reduce-context" exactTabLabel="explainability.labels.exactSource" copyLabel="explainability.actions.copyExactReduceContext" description="explainability.messages.reduceContextSourceNotice" />
        <CapturedContentViewer buttonLabel="explainability.actions.viewReducePrompt" title={t("explainability.labels.reducePrompt")} content={summary.exactPrompt} testId="exact-drift-reduce-prompt" />
        {summary.generated ? <CapturedContentViewer buttonLabel="explainability.actions.viewRawReduceResponse" title={t("explainability.labels.rawReduceResponse")} content={summary.rawResponse} testId="raw-drift-reduce-response" preview={false} /> : null}
      </div>
...
function PrimerFold({ fold }: { fold: DriftPrimerFoldView }): React.ReactElement {
  ...
  return <div className="min-w-0 rounded border bg-background/50 p-2"><button type="button" className="flex w-full min-w-0 items-center gap-2 text-left text-xs" aria-expanded={expanded} aria-label={t(expanded ? "explainability.actions.collapsePrimerFold" : "explainability.actions.expandPrimerFold", { fold: fold.foldIndex + 1 })} onClick={() => setExpanded((value) => !value)}>{expanded ? <ChevronDown className="size-3.5 shrink-0" /> : <ChevronRight className="size-3.5 shrink-0" />}<span className="min-w-0 flex-1 font-medium">{t("explainability.labels.foldValue", { value: fold.foldIndex + 1 })}</span><span className="shrink-0 text-muted-foreground">{t("graph.counts.countReport", { count: fold.reportIds.length })}{fold.score === undefined ? "" : ` · ${t("explainability.labels.scoreValue", { value: fold.score.toFixed(1) })}`}{fold.followUpCount === undefined ? "" : ` · ${t("explainability.counts.countFollowUp", { count: fold.followUpCount })}`}</span></button>{expanded ? <div className="mt-2 min-w-0 space-y-2 border-t pt-2"><p className="break-all font-mono text-[10px] text-muted-foreground">{fold.reportIds.length === 0 ? t("explainability.labels.0Reports") : fold.reportIds.join("\n")}</p><div className="flex flex-wrap gap-2"><CapturedContentViewer buttonLabel="explainability.actions.viewPrimerPrompt" title={t("explainability.labels.primerPromptTitle", { fold: fold.foldIndex + 1 })} content={fold.exactPrompt} testId={`drift-fold-${fold.foldIndex}-prompt`} />{fold.completed ? <CapturedContentViewer buttonLabel="explainability.actions.viewRawPrimerResponse" title={t("explainability.labels.primerRawResponseTitle", { fold: fold.foldIndex + 1 })} content={fold.rawResponse} testId={`drift-fold-${fold.foldIndex}-response`} preview={false} /> : null}<CapturedContentViewer buttonLabel="explainability.actions.viewParsedAnswer" title={t("explainability.labels.primerParsedAnswerTitle", { fold: fold.foldIndex + 1 })} content={fold.intermediateAnswer} testId={`drift-fold-${fold.foldIndex}-answer`} preview={false} /><CapturedContentViewer buttonLabel="explainability.actions.viewParsedFollowUps" title={t("explainability.labels.primerFollowUpsTitle", { fold: fold.foldIndex + 1 })} content={fold.followUpQueries?.join("\n") ?? null} testId={`drift-fold-${fold.foldIndex}-followups`} preview={false} /></div><LlmMetrics view={fold} /></div> : null}</div>
}
...
function ActionAttempt({ attempt, index }: { attempt: DriftActionAttemptView; index: number }): React.ReactElement {
  ...
  return <section className="min-w-0 rounded border bg-muted/20 p-2" aria-label={t("explainability.labels.actionAttempt", { action: attempt.actionId, attempt: index + 1 })}><div className="flex items-center justify-between gap-2 text-xs"><span className="font-medium">{t("explainability.labels.attemptDepth", { attempt: index + 1, depth: attempt.depthIndex + 1 })}</span><Badge variant="outline">{status}</Badge></div><dl className="mt-2 grid grid-cols-2 gap-2 text-xs"><Metric label="explainability.labels.score" value={attempt.score === undefined ? t("explainability.labels.noFiniteScore") : attempt.score.toFixed(1)} />{attempt.followUpCount === undefined ? null : <Metric label="explainability.labels.followUps" value={attempt.followUpCount} />}</dl><div className="mt-2 flex min-w-0 flex-wrap gap-2"><CapturedContentViewer buttonLabel="explainability.actions.viewQuery" title={t("explainability.labels.actionIdQuery", { id: attempt.actionId })} content={attempt.query} testId={`drift-action-${attempt.actionId}-${index}-query`} preview={false} /><CapturedContentViewer buttonLabel="explainability.actions.viewLocalContext" title={t("explainability.labels.actionIdLocalContext", { id: attempt.actionId })} content={attempt.context} testId={`drift-action-${attempt.actionId}-${index}-context`} /><CapturedContentViewer buttonLabel="explainability.actions.viewActionPrompt" title={t("explainability.labels.actionIdPrompt", { id: attempt.actionId })} content={attempt.exactPrompt} testId={`drift-action-${attempt.actionId}-${index}-prompt`} />{attempt.rawResponse === null ? null : <CapturedContentViewer buttonLabel="explainability.actions.viewRawActionResponse" title={t("explainability.labels.actionIdRawResponse", { id: attempt.actionId })} content={attempt.rawResponse} testId={`drift-action-${attempt.actionId}-${index}-response`} preview={false} />}<CapturedContentViewer buttonLabel="explainability.actions.viewParsedAnswer" title={t("explainability.labels.actionIdParsedAnswer", { id: attempt.actionId })} content={attempt.answer} testId={`drift-action-${attempt.actionId}-${index}-answer`} preview={false} /><CapturedContentViewer buttonLabel="explainability.actions.viewGeneratedFollowUps" title={t("explainability.labels.actionIdGeneratedFollowUps", { id: attempt.actionId })} content={attempt.followUpQueries?.join("\n") ?? null} testId={`drift-action-${attempt.actionId}-${index}-followups`} preview={false} /></div><LlmMetrics view={attempt} /></section>
}
