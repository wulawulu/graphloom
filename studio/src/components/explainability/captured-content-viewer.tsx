...
export function CapturedContentViewer({
  ...
  copyLabel,
  description,
}: CapturedContentViewerProps): React.ReactElement | null {
    ...
    <Collapsible>
      ...
      <CollapsibleContent className="mt-2 min-w-0 overflow-hidden rounded-md border bg-background/60 p-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={copy} aria-label={copyLabel === undefined ? t("explainability.actions.copyExactTitle", { title }) : t(copyLabel)}><Copy className="size-3.5" />{copied ? t("common.copied") : t("common.copy")}</Button>
        </div>
