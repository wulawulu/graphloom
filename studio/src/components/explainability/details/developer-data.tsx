...
export function DeveloperData({ event }: { event: ExplainabilityEventPayload }): React.ReactElement {
  ...
  return (
    <details className="rounded-md border bg-muted/20 p-3">
      <summary className="cursor-pointer text-xs font-medium text-muted-foreground">{t("explainability.labels.developerData")}</summary>
      <p className="mt-2 font-mono text-[10px] text-muted-foreground">{t("explainability.labels.rawJson")}</p>
