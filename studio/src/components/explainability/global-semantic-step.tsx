  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="min-w-0 rounded border bg-background/50">
      ...
      <CollapsibleContent className="min-w-0 space-y-3 border-t p-3">
        ...
        <CapturedContentViewer buttonLabel="explainability.actions.viewMapContext" title={t("explainability.labels.mapContextTitle", { batch: batch.batchIndex + 1 })} content={batch.exactContext} testId={`exact-map-context-${batch.batchIndex}`} />
      </CollapsibleContent>
    </Collapsible>
  )
}
    ...
    <Collapsible open={open} onOpenChange={setOpen} className="min-w-0 rounded border bg-background/50">
      ...
      <CollapsibleContent className="min-w-0 space-y-4 border-t p-3">
        ...
        <div className="flex min-w-0 flex-wrap gap-2">
          <CapturedContentViewer buttonLabel="explainability.actions.viewMapContext" title={t("explainability.labels.mapContextTitle", { batch: batch.batchIndex + 1 })} content={batch.exactContext} testId={`analysis-exact-map-context-${batch.batchIndex}`} />
          <CapturedContentViewer buttonLabel="explainability.actions.viewMapPrompt" title={t("explainability.labels.mapPromptTitle", { batch: batch.batchIndex + 1 })} content={batch.exactPrompt} testId={`exact-map-prompt-${batch.batchIndex}`} />
          <CapturedContentViewer buttonLabel="explainability.actions.viewRawMapResponse" title={t("explainability.labels.mapRawResponseTitle", { batch: batch.batchIndex + 1 })} content={batch.rawResponse} testId={`raw-map-response-${batch.batchIndex}`} preview={false} />
        </div>
...
function EvidenceReductionContent({ summary }: { summary: Extract<GlobalSemanticStep, { kind: "evidence-reduction" }>["summary"] }): React.ReactElement {
  ...
  return (
    <div className="mt-3 min-w-0 space-y-3">
      ...
      <CapturedContentViewer buttonLabel="explainability.actions.viewReduceContext" title={t("explainability.labels.reduceContext")} content={summary.exactContext} testId="exact-reduce-context" />
    </div>
  )
}
...
function AnswerGenerationContent({ summary }: { summary: Extract<GlobalSemanticStep, { kind: "global-answer-generation" }>["summary"] }): React.ReactElement {
  ...
  return (
    <div className="mt-3 min-w-0 space-y-3">
      ...
      <div className="flex min-w-0 flex-wrap gap-2"><CapturedContentViewer buttonLabel="explainability.actions.viewReducePrompt" title={t("explainability.labels.reducePrompt")} content={summary.exactPrompt} testId="exact-reduce-prompt" /><CapturedContentViewer buttonLabel="explainability.actions.viewRawReduceResponse" title={t("explainability.labels.rawReduceResponse")} content={summary.rawResponse} testId="raw-reduce-response" preview={false} /></div>
    </div>
  )
}
