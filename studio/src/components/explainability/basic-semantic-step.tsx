...
function StepSummary({ step }: { step: BasicSemanticStep }): React.ReactElement {
  ...
  if (step.kind === "text-retrieval") {
    if (step.summary.status === "skipped") return <p className="mt-1 text-xs text-muted-foreground">{t("Retrieval skipped — empty query")}</p>
    if (step.summary.status === "embedding") return <p className="mt-1 text-xs text-muted-foreground">{t("Embedding query...")}</p>
    if (step.summary.status === "embedding_ready") return <p className="mt-1 text-xs text-muted-foreground">{t("Embedding ready · waiting for ANN results")}</p>
    if (step.summary.status === "waiting") return <p className="mt-1 text-xs text-muted-foreground">{t("Waiting for retrieval")}</p>
    return <p className="mt-1 text-xs text-muted-foreground">{t("{{count}} text unit retrieved", { count: step.summary.candidates.length })}</p>
  }
  if (step.kind === "basic-context-assembly") {
    const summary = step.summary
    if (summary.status === "waiting") return <p className="mt-1 text-xs text-muted-foreground">{t("Waiting for context assembly")}</p>
    if (summary.status === "assembling") return <p className="mt-1 text-xs text-muted-foreground">{t("Assembling Sources context")}</p>
    return <p className="mt-1 text-xs text-muted-foreground">{t("{{candidates}} candidates · {{included}} included", { candidates: summary.candidateCount ?? summary.candidates.length, included: summary.selectedCount ?? 0 })}{summary.tokenBudgetExcludedCount === undefined ? "" : ` · ${t("{{count}} excluded by token budget", { count: summary.tokenBudgetExcludedCount })}`}</p>
  }
  const status = t(step.summary.status === "generated" ? "Answer generated" : step.summary.status === "generating" ? "Generating" : "Waiting")
...
function StepContent({ step }: { step: BasicSemanticStep }): React.ReactNode {
  const { t } = useTranslation()
  if (step.kind === "text-retrieval") {
    if (step.summary.status === "skipped") return <p className="mt-3 rounded border bg-muted/20 p-3 text-xs">{t("Embedding and ANN search were not invoked because the query was empty.")}</p>
    const metadata = [
      step.summary.model,
  ...
  if (step.kind === "basic-context-assembly") {
    ...
    return (
      <div className="mt-3 min-w-0 space-y-3">
        {summary.status === "waiting" ? null : <p className="text-[11px] text-muted-foreground">{t("Basic preserves source-table order after ANN matching; token fitting stops at the first over-budget row.")}</p>}
...
function CandidateList({ candidates, mode }: { candidates: ExplainabilityCandidate[]; mode: "retrieved" | "decision" }): React.ReactElement | null {
  ...
  return (
    <div className="min-w-0 space-y-2">
      ...
      {candidates.length > INITIAL_CANDIDATES ? <Button variant="ghost" size="sm" aria-expanded={showAll} aria-label={showAll ? t("Show fewer Basic text units") : t("Show all {{count}} Basic text units", { count: candidates.length })} onClick={() => setShowAll((value) => !value)}>{showAll ? t("Show fewer text units") : t("Show all {{count}} text units", { count: candidates.length })}</Button> : null}
    </div>
  )
}
...
function CandidateRow({ candidate, mode }: { candidate: ExplainabilityCandidate; mode: "retrieved" | "decision" }): React.ReactElement {
  const { t } = useTranslation()
  const label = candidate.short_id === undefined ? candidate.id : t("Text Unit {{id}}", { id: candidate.short_id })
  const decision = t(candidate.selected ? "Included" : candidate.reason === "token_budget" ? "Not included after token-budget stop" : "Not included")
  return (
    <div className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] gap-x-2 gap-y-1 overflow-hidden px-2 py-2 text-xs">
      <span className="row-span-2 pt-0.5">{mode === "retrieved" || !candidate.selected ? <Circle className="size-3.5 text-muted-foreground" aria-label={mode === "retrieved" ? t("Retrieved") : decision} /> : <Check className="size-3.5 text-success" aria-label={t("Included")} />}</span>
      ...
      <Badge variant="outline" className="max-w-48 shrink-0 truncate" title={mode === "retrieved" ? t("Retrieved") : decision}>{mode === "retrieved" ? t("Retrieved") : decision}</Badge>
      <div className="col-span-2 col-start-2 flex min-w-0 gap-3 text-[10px] text-muted-foreground">{candidate.rank === undefined ? null : <span>{t("ANN rank {{value}}", { value: candidate.rank })}</span>}{candidate.score === undefined ? null : <span>{t("Score {{value}}", { value: candidate.score.toFixed(4) })}</span>}</div>
    </div>
  )
}
