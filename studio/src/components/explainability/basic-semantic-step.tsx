...
function StepContent({ step }: { step: BasicSemanticStep }): React.ReactNode {
  ...
  if (step.kind === "basic-context-assembly") {
    ...
    return (
      <div className="mt-3 min-w-0 space-y-3">
        ...
        {summary.status === "completed" ? <CapturedContentViewer buttonLabel="explainability.actions.viewBasicContext" title={t("explainability.labels.basicContext")} content={summary.exactContext} testId="exact-basic-context" exactTabLabel="explainability.labels.exactInput" copyLabel="explainability.actions.copyExactBasicContext" /> : null}
      </div>
    )
  }
  const summary = step.summary
  return (
    <div className="mt-3 min-w-0 space-y-3">
      ...
      {summary.status === "waiting" ? null : <div className="flex min-w-0 flex-wrap gap-2"><CapturedContentViewer buttonLabel="explainability.actions.viewBasicPrompt" title={t("explainability.labels.basicPrompt")} content={summary.exactPrompt} testId="exact-basic-prompt" />{summary.status === "generated" ? <CapturedContentViewer buttonLabel="explainability.actions.viewRawBasicResponse" title={t("explainability.labels.rawBasicResponse")} content={summary.rawResponse} testId="raw-basic-response" preview={false} /> : <span className="self-center text-xs text-muted-foreground">{t("explainability.labels.waitingForProviderResponse")}</span>}</div>}
    </div>
  )
}
