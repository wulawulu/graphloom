...
export function CommunityList({ onSelect }: EntityListProps): React.ReactElement {
  ...
  return <GraphListFrame filters={<><Input type="number" aria-label={t("graph.labels.communityLevelFilter")} placeholder={t("explainability.labels.level")} value={draftLevel} onChange={(event) => setDraftLevel(event.target.value)} /><Input type="number" aria-label={t("graph.labels.communityParentFilter")} placeholder={t("graph.labels.parent")} value={draftParent} onChange={(event) => setDraftParent(event.target.value)} /><Button type="submit" variant="outline" size="icon" aria-label={t("graph.actions.applyCommunityFilters")}><Filter /></Button></>} onApplyFilters={applyFilters} items={items} cursor={cursor} loading={loading} error={error} onLoadMore={loadMore} render={(item) => <button key={item.id} type="button" className="flex w-full items-center justify-between rounded-md border p-2.5 text-left hover:bg-accent" onClick={() => onSelect(item.id)}><span className="min-w-0"><span className="flex items-center gap-2 truncate text-sm font-medium">{item.title}<Badge variant="outline">L{item.level}</Badge></span><span className="block text-[10px] text-muted-foreground">{t("graph.status.communityHierarchy", { shortId: item.short_id, parent: item.parent, children: item.children.join(", ") || t("graph.status.none") })}</span><span className="line-clamp-1 text-[11px] text-muted-foreground">{item.report?.summary ?? t("graph.labels.noReportSummary")}</span></span><ChevronRight className="size-4 shrink-0" /></button>} />
}
