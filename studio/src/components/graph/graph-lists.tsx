...
export function EntityList({ onSelect }: EntityListProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    void listEntities({ type: appliedFilters.entityType || undefined, community: appliedFilters.community || undefined, limit: 50 }, controller.signal)
  ...
  const loadMore = (): void => {
    ...
    setLoading(true); setError(false)
    void listEntities({ type: appliedFilters.entityType || undefined, community: appliedFilters.community || undefined, limit: 50, after: cursor }, controller.signal)
...
export function RelationshipList({ onSelect }: EntityListProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    void listRelationships({ source: appliedFilters.source || undefined, target: appliedFilters.target || undefined, limit: 50 }, controller.signal).then((response) => { if (request.current === controller && !controller.signal.aborted) { setItems(response.items); setCursor(response.next_cursor) } }).catch((reason: unknown) => { if (request.current === controller && !isAbort(reason)) setError(true) }).finally(() => { if (request.current === controller) { request.current = null; setLoading(false) } })
    return () => { request.current?.abort(); if (request.current === controller) request.current = null }
  }, [appliedFilters])
  ...
  const loadMore = (): void => { if (cursor === null) return; request.current?.abort(); const controller = new AbortController(); request.current = controller; setLoading(true); setError(false); void listRelationships({ source: appliedFilters.source || undefined, target: appliedFilters.target || undefined, limit: 50, after: cursor }, controller.signal).then((response) => { if (request.current === controller && !controller.signal.aborted) { setItems((current) => [...current, ...response.items]); setCursor(response.next_cursor) } }).catch((reason: unknown) => { if (request.current === controller && !isAbort(reason)) setError(true) }).finally(() => { if (request.current === controller) { request.current = null; setLoading(false) } }) }
  return <GraphListFrame filters={<><Input aria-label="Relationship source filter" placeholder="Exact source title" value={draftSource} onChange={(event) => setDraftSource(event.target.value)} /><Input aria-label="Relationship target filter" placeholder="Exact target title" value={draftTarget} onChange={(event) => setDraftTarget(event.target.value)} /><Button type="submit" variant="outline" size="icon" aria-label="Apply relationship filters"><Filter /></Button></>} onApplyFilters={applyFilters} items={items} cursor={cursor} loading={loading} error={error} onLoadMore={loadMore} render={(item) => <button key={item.id} type="button" className="flex w-full items-center justify-between rounded-md border p-2.5 text-left hover:bg-accent" onClick={() => onSelect(item.id)}><span className="min-w-0"><span className="block truncate text-sm font-medium">{item.source} → {item.target}</span><span className="text-[11px] text-muted-foreground">weight {item.weight ?? "—"} · rank {item.rank ?? "—"}</span></span><ChevronRight className="size-4 shrink-0" /></button>} />
}
...
interface GraphListFrameProps<T> { filters: React.ReactNode; onApplyFilters: () => void; items: T[]; cursor: string | null; loading: boolean; error: boolean; onLoadMore: () => void; render: (item: T) => React.ReactNode }
function GraphListFrame<T>({ filters, onApplyFilters, items, cursor, loading, error, onLoadMore, render }: GraphListFrameProps<T>): React.ReactElement {
  const submit = (event: React.FormEvent<HTMLFormElement>): void => { event.preventDefault(); onApplyFilters() }
  return <div className="flex min-h-0 flex-1 flex-col gap-3"><form className="grid grid-cols-[1fr_1fr_auto] gap-2" onSubmit={submit}>{filters}</form>{error ? <p className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-xs text-red-300">Graph list is unavailable.</p> : null}<ScrollArea className="min-h-0 flex-1"><div className="space-y-1.5 pr-2">{items.map(render)}{!loading && !error && items.length === 0 ? <p className="py-12 text-center text-xs text-muted-foreground">No graph items match this filter.</p> : null}{cursor !== null ? <Button variant="outline" size="sm" className="w-full" disabled={loading} onClick={onLoadMore}>Load more</Button> : null}</div></ScrollArea></div>
}
