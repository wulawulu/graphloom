...
export function GraphReferenceCard({ onOpen, reference }: GraphReferenceCardProps): React.ReactElement {
  ...
  const title = community
    ? communityDisplayTitle(reference.value, t("graph.community.communityNumber", { shortId: reference.value.short_id }))
    : value.title
  const secondary = community
    ? t("graph.community.referenceMetadata", { shortId: reference.value.short_id, level: reference.value.level })
  ...
  const ariaLabel = t(community ? "graph.navigation.openCommunity" : "graph.navigation.openEntity", { title })
      ...
      <HoverCardTrigger asChild>
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 block break-words text-sm font-medium">{title}</span>
