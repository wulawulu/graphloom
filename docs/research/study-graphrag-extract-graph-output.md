...
当前 GraphLoom 已满足缓存兼容，但 `entities.parquet` 的差异说明默认模式尚未满足完整的输出行为兼容。
物理 Parquet 复制仍不是要求。`relationships.parquet` 没有类似问题，因为关系摘要和连接在两侧都使用
完整的 `("source", "target")` 身份。

## 兼容性决策（2026-07-16 更新）

默认兼容模式应复现固定 GraphRAG 基线的 title-only summary join，包括多类型同标题时的结果基数和字段
组合。这不是对该算法语义正确性的认可，而是为了先建立可逐项比较的行为基线。

当前“一条输入记录对应一条输出记录”的实现应保留为未来显式优化模式。该模式必须有明确配置名、与
兼容模式分离的验证样例，并记录产物差异；在模式边界完成前，当前默认输出与 GraphRAG 的差异属于实现
缺口，不能再表述为已接受的默认偏差。
...
- `relationships.parquet` 的语义行顺序与 GraphRAG 相同，因为社区检测的“保留最后一条重复无向边”
  会把顺序差异转化为边权差异；
- 默认兼容模式的实体行多重集合与 GraphRAG 相同，包括 title-only join 产生的记录；
