...
对于不支持 GraphRAG JSON Schema entity-type 请求的模型，可运行：
...
`--no-discover-entity-types` 是真实 GraphRAG CLI/API 模式：使用配置的默认
entity types，只跳过 structured-output discovery 请求。仅当 provider
拒绝 JSON Schema response format 时使用。
...
Random 与 Auto 让 `first.txt` 成为唯一 1,000-token 候选，避免伪装 Python
pandas RNG 与 Rust RNG 可共享实现无关 seed。多候选算法由确定性离线测试
覆盖；live 验收验证每种模式的真实编排、completion 请求和逐字节 prompt。
Auto 还在两边执行真实 `ollama/bge-m3` embedding。
...
默认输出为仓库相对路径 `prompt_tune_real_llm/typed-top`。已有运行不会覆盖；
`--clean` 只删除指定 run-name 目录。
