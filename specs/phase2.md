...
DRIFT 对注入固定 RNG 的 fixture 精确比较选择、状态节点/边和 Reduce answers。
...
重复 action、数量、深度或请求契约不一致仍必须失败。Python 与 Rust 测试使用同一显式位置轨迹，
分别 monkeypatch Python 和注入 Rust 私有 RNG，并独立断言相同的选择、节点、边和 Reduce answer
golden；这不是固定轨迹的跨语言端到端 CLI 或完整 Local/Reduce 请求比较。不得假设相同 seed 会
产生相同 shuffle。
