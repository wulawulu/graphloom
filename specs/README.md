...
| 状态             | Phase 1 主体已实现，规格已按 Step 10 收尾更新             |
| 实现语言           | Rust                                       |
| 参考项目           | Microsoft GraphRAG                         |
| 参考 tag          | `v3.1.0`                                  |
| 固定基准提交         | `2077c4205add901e6594aced159fca81b7a6d522` |
...
当前只实施 Phase 1。Phase 1 已包含可运行的 `graphloom init` 和 `graphloom index`，但不包含 query、update、prompt-tune 或 Python 兼容测试。Phase 1 的公共接口不得阻止 Phase 2 和 Phase 3 的实现。
...
Phase 1 的 Rust 端验收先覆盖 GraphRAG 3.1.0 风格配置、prompt、标准 Parquet schema、LanceDB 结构和本地 OpenAI-compatible stub 端到端索引。Python 互读/互跑兼容测试属于后续单独阶段，不作为 Step 10 的完成条件。
...
`output/`、`cache/`、`logs/` 和 LanceDB 路径来自 `settings.yaml` 的 storage/cache/reporting/vector_store 配置。`init` 不预创建这些运行产物目录；`index` 在通过路径安全检查后按需创建或清理。
...
`graphloom index` 清理 output 前必须拒绝危险路径，包括：
...
-重复运行时按 ID upsert；
-不因重跑产生重复向量；
-full index 开始前必须 reset 所有 GraphLoom 管理的 embedding index；
...
4. 校验 destructive output path 安全。
5. 正式运行时清理 output storage。
6. reset 所有受管 LanceDB embedding index。
7. 创建 providers。
8. 注册 built-in workflows。
9. 创建 standard pipeline。
10. 依次运行 workflows。
11. 输出 progress。
12. 输出最终统计。
13. 任一步失败时返回非零退出码。
...
`--dry-run` 只加载配置、替换环境变量、解析 paths、执行安全和配置验证、输出 redacted summary 与 workflow 列表；不得调用 LLM、创建 output/cache/logs/LanceDB、写 Parquet 或修改 cwd。
...
* `graphloom init`；
* `graphloom index --dry-run` 零副作用；
* `graphloom index` 完整 Step 5 至 Step 9；
* 真实 Parquet 输出；
* 真实 LanceDB 输出；
* full rerun 清理旧 output 和受管 vector index；
* `--no-cache` 不删除已有 cache。

## 25.4 Python 兼容测试（后续阶段）

Python compatibility test 不属于 Phase 1 Step 10 的完成条件。后续建立独立命令：
...
Python compatibility test 不属于 Step 10。本阶段生成的配置、prompt、Parquet 和 LanceDB 结构仍以 GraphRAG 3.1.0 兼容为目标，但 Python 互操作验证留给后续独立阶段。
...
-写入 LanceDB；
-第二次 full index 会清理旧 output 和受管 vector index，不 append 旧数据；
...
Python GraphRAG 互读互跑测试暂不作为 Phase 1 Step 10 验收项。
