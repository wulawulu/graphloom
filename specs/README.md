...
不得参考基准提交之后的 GraphRAG 代码来改变 v3.1.0 核心 indexing 行为，除非规范被显式更新。表中的 `cache/operation 研究 pin` 是针对演进后的 cache 协议和已记录 operation 对照的显式例外，不代表基准自动漂移到 GraphRAG main。
...
GraphLoom 的首要兼容目标是行为兼容：相同输入、配置、prompt 和模型响应应产生相同的 workflow 决策和逻辑结果。在这一基线上再引入优化；优化不得无意改变默认兼容行为。
...
* LanceDB collection 名称与向量记录逻辑结构；
* snapshot 配置行为；
* cache 命名空间和缓存键语义；
* workflow 配置覆盖机制。
...
1. **Workflow 行为：**workflow 名称、顺序、分块、prompt、解析、图和社区结果。
2. **协议互操作：**cache namespace、请求规范化、缓存键和响应 envelope 可直接复用。
3. **逻辑数据互操作：**表名、列名、逻辑类型、引用关系和向量记录语义兼容。
4. **物理存储互操作：**Parquet writer/Arrow 物理表示和 LanceDB on-disk format 可由对方运行时直接打开。

Phase 1 以前三层为验收重点。第四层是明确记录的存储兼容加固项，不得用其当前缺口否定已经验证的 workflow 和 cache 行为，也不得把它误写成已经完成。
...
* Parquet 文件二进制完全相同；
* 行组大小和压缩后的字节完全相同；
* 不同 LanceDB 版本之间的 on-disk database 目录直接互开；
* LLM 生成文本逐字相同；
...
未经规范修改，不得新增、合并或删除 crate。CLI 归属暂以 `graphloom` crate 为准；若后续将 CLI 代码拆分或迁移，必须同步更新本节。
...
* callbacks；
* reporting；
* dry-run、no-cache、console callback 和 file logging；
* CLI 结构化错误和 secret redaction；
* prompt 加载和渲染；
  -各个 provider 的最终组装。

`crates/graphloom/src` 当前按 Rust 运行时边界组织，而不是逐目录镜像 Python package：
...
workflow 与 operation 名称应尽量与基准 Python 实现对应，以便行为对照；Rust 模块边界以职责和依赖方向为准。
...
缓存键的 canonical payload 由 LLM/operation 边界定义，缓存 crate 只负责命名空间和键值存取。

## 4.2 `graphloom-chunking`
...
* Polars DataFrame 和 Arrow 之间的适配；
  -逐行流式表读写；
* table namespace；
* append、truncate 和完整对象替换。

该 crate 不得包含任何实体提取、社区生成或 workflow 业务。

## 4.6 `graphloom-vectors`
...
不得引入生产级 `MemoryVectorStore` 或 `VectorStoreType::Memory`。测试可以使用私有 mock/recording store，但不得公开导出、注册到 factory 或成为配置项。
...
```text
graphloom ──▶ graphloom-cache ──────▶ graphloom-storage
    ├──────▶ graphloom-chunking
    ├──────▶ graphloom-input ───────▶ graphloom-storage
    ├──────▶ graphloom-llm ─────────▶ graphloom-cache
    │                    └──────────▶ graphloom-storage
    ├──────▶ graphloom-storage
    └──────▶ graphloom-vectors
```
...
尤其必须在同一个 Rust workspace 内统一 Arrow、Parquet、Polars 和 LanceDB 相关版本，避免 Rust 组件之间出现不兼容的 Arrow 主版本。该约束不等于要求与 Python GraphRAG 使用相同的底层 crate/package 版本。
...
| Hash                | SHA-512（GraphRAG IDs）；SHA-256（cache key） |
| ID                  | UUID v4                     |
| 日志                  | tracing                     |
| CLI                 | clap                        |
| library error       | thiserror                   |
| CLI error            | thiserror 结构化错误，外层可追加上下文 |
...
1. 从 `IndexPipelineContext` 中获取 provider。
2. 从具名表读取输入。
3. 执行转换。
4. 将正式输出写回具名表。
...
## 7.2 IndexWorkflowOutput 不是下游数据源
...
Phase 2 的 similarity search 应在 query 需求明确后扩展，不在 Phase 1 中预留猜测性接口。
...
# 9. IndexPipelineContext

`IndexPipelineContext` 通过 `IndexRuntimeServices` 持有 active workflows 实际需要的服务：
...
| ------------------------- | -------------------------------------- |
| `stats`                   | 当前运行统计                                 |
| `input_reader`            | 当前输入文档 reader                          |
| `output_storage`          | snapshot 和其他长期文件输出                     |
| `output_table_provider`   | 当前索引正式表                                |
| `cache`                   | LLM 和其他可缓存结果                           |
| `vector_store`            | Step 9 向量写入后端，Phase 1 正式实现为 LanceDB       |
| `models`                  | 按 active workflow requirements 构建的模型注册表      |
| `callbacks`               | progress、error、warning 和 workflow 生命周期 |
| `project_root`            | prompt 和项目相对路径解析基准，不得依赖 process cwd      |

Phase 1 不为未来 update workflow 预留未使用的 `previous_table_provider` 或任意 `state` 字段。Phase 3 应建立独立、明确的 update runtime 契约；当前继续保留：
...
`output/`、`cache/`、`logs/` 和 LanceDB 路径来自 `settings.yaml` 的 storage/cache/reporting/vector_store 配置。`init` 不预创建这些运行产物目录；`index` 在通过路径安全检查后按需创建，并只替换当前 workflow 管理的表或向量 collection，不递归清空整个 output。
...
`model_provider` 是 GraphRAG 3.1.0 canonical 字段，旧 `type` 字段作为 GraphLoom legacy alias 保留。`retry` nested 配置优先于 legacy flat retry 字段。
...
API key 为空或仍为 `<API_KEY>` 时，正式 index preflight 必须失败。`api_key`、token、authorization、password、secret 等敏感字段不得出现在 Debug、日志或错误信息中。
...
## 10.5 Project config loader 和路径安全

CLI 入口必须支持从项目 root 搜索配置文件，顺序为：
...
也允许 library API 直接接收具体配置文件路径。配置文件格式只支持 YAML、YML 和 JSON，不支持 TOML。

`.env` 查找基于配置文件所在目录，不存在时不报错。变量替换发生在配置反序列化前，支持：
...
进程环境变量优先于项目 `.env`。缺少变量或 malformed `.env` 必须 fail-fast，错误中包含变量名或行号。loader 不得调用 `set_current_dir` 或 `set_var`。

所有项目相对路径必须基于 settings 文件所在目录解析，而不是 process cwd。`ProjectPaths` 至少集中包含：
...
`graphloom index` 写入 output 或 reset 受管向量 collection 前必须拒绝危险路径，包括：

* `output_dir == project root`；
* `output_dir == input_dir/cache_dir/reporting_dir`；
* `output_dir` 是 project root、input、cache 或 reporting 的祖先；
* filesystem root；
* 用户 home 根目录。

不得通过字符串前缀做路径安全判断。已存在路径应尽可能 canonicalize；不存在路径至少做 lexical normalize。
...
默认模板由 `graphloom init` 从内置 assets 写入项目目录：
...
1. 配置中明确指定的文件或 inline prompt；
2. 项目根目录 `prompts/` 中的覆盖文件；
3. GraphLoom 内置默认模板。

相对 prompt 路径必须基于 project root 解析，不得依赖 process cwd。绝对 prompt 路径保持绝对。loader 不得把任意 inline prompt 文本误判为文件路径。
...
-保留已经完成的前序写入；
-不自动回滚整个索引；
-当前 workflow 在失败前已经完成的写入也可能保留，因此失败后的 output 可能是部分更新状态；
-错误中必须包含 workflow 名称。
...
* `InputConfig`；
* `IndexPipelineContext::input_reader()`。
...
-同表读写必须先在内存中完成转换，再以一次完整表写入替换目标对象；当前文件后端不承诺跨文件系统崩溃一致性的原子 rename。
...
-裁剪并排序为最终列；
-同表读写使用完整表替换，不声明 workflow 级事务或失败回滚。
...
1. 从原始文件读取。
2. 在内存中累计待写 DataFrame。
3. `close()` 时序列化完整结果并通过 `Storage::set` 替换目标对象。
4. `abort()` 时丢弃尚未写入的内存数据。
...
读取 entities
同时生成新的 entities
最后完整写回 entities.parquet
```

该接口当前不是事务边界：文件后端直接写目标路径，不使用 staging 文件或原子 rename；`Storage::set` 已成功返回的写入不会因后续 workflow 失败而回滚。需要崩溃一致性时必须另立设计，不能从当前 `TableProvider` 契约推断。
...
Embedding request cache key 至少包含：

-输入文本；
-embedding dimensions 等 request 参数。

Embedding model instance 隔离不由 request key 完成，而由 `Cache::child(model_instance_name)` 完成。workflow 不得把 root cache 直接传入 embedding operation。
...
-重复运行时按 ID upsert；
-不因重跑产生重复向量；
-当配置的 workflow 需要向量存储时，在 pipeline 开始前 reset 所有 GraphLoom 管理的 embedding index；
-reset 只作用于指定 LanceDB table，不得递归删除任意 `db_uri`；
...
LanceDB upsert 保持幂等，但不得用于掩盖 source 数据错误；同一 embedding field 的 source id 必须全表唯一，重复 id 为 fatal input error。
...
```text
graphloom init --root <path> --model <model> --embedding <model> --force
```
...
`init` 不创建 `output/`、`cache/`、`logs/` 或 LanceDB 目录。这些是 `index` 的运行产物，必须由 `index` 在路径安全检查通过后按需创建。

`--force` 只覆盖 GraphLoom 管理的默认文件：

* `settings.yaml`；
* `.env`；
* 同名默认 prompt。

不得删除 `input/` 中的用户文件、root 下未知文件或用户额外创建的 prompt。
...
```text
graphloom index --root <path> --method standard --verbose --dry-run --cache --no-cache --skip-validation
```
...
3. 执行 preflight validation。
4. 校验受管 output/vector 写路径安全。
5. 注册 built-in workflows 并创建 standard pipeline。
6. 根据 active workflow requirements 创建所需 providers。
7. active workflow 需要向量存储时，reset 受管 LanceDB embedding collections。
8. 依次对 active output 直接运行 workflows；每个 workflow 只替换自己管理的表。
9. 输出 progress。
10. 输出最终统计。
11. 任一步失败时返回非零退出码，且不回滚已经完成的表或向量写入。

`index` 不创建隔离 generation，不执行最终 publish/activate，也不递归清空 output。未被 active workflows 触及的文件和表保持原样。

仅支持 `--method standard`。其他 method 必须返回明确错误，不得创建 fake fast/update pipeline。

`--dry-run` 加载配置、替换环境变量、解析 paths、执行与正式运行相同的非破坏性安全和可选配置验证，并输出 redacted summary 与 workflow 列表。未使用 `--skip-validation` 时，它会向 active workflows 所需的 completion/embedding model 各发送一次短小且不使用 cache 的连通性请求；不得创建 output/cache/logs/LanceDB、写 Parquet 或修改 cwd。

`--no-cache` 只禁用本次运行的 cache，不得删除已有 cache。
...
CLI 使用结构化错误区分配置、路径、provider、storage、cache、runtime 和 indexing 失败；最外层可以追加运行上下文，但不得抹掉底层类型或泄漏 secret。
...
workflow-level pipeline 测试可以注入 Mock LLM，返回固定：
...
CLI 端到端测试必须通过真实 `graphloom` binary、真实 OpenAI adapter 和本地 OpenAI-compatible HTTP stub 运行，不得在生产配置中增加 `model_provider: mock`，不得绕过 completion/embedding adapter，不得预写最终表。

CLI 端到端测试至少覆盖：

* `graphloom init`；
* `graphloom index --dry-run` 不产生文件系统或 cache/LanceDB 写入；默认 full validation 的模型连通性请求除外；
* `graphloom index` 完整 Step 5 至 Step 9；
* 真实 Parquet 输出；
* 真实 LanceDB 输出；
* 重跑覆盖 active workflows 管理的表并 reset 受管 vector index，同时保留无关 output 文件；
* `--no-cache` 不删除已有 cache。
...
* `graphloom index`；
  ...
  -本地 OpenAI-compatible stub 端到端测试；
...
graphloom index --root ./example --dry-run
```
...
-写入 LanceDB；
-第二次 standard index 会覆盖 active workflows 管理的表并 reset 受管 vector index，不 append 这些受管数据；不相关 output 文件保持原样；
-启用 cache 后可以复用 LLM 结果；
-`--no-cache` 不删除已有 cache；
-`--dry-run` 不产生文件系统或 cache/LanceDB 写入；默认 full validation 仍会执行模型连通性请求；
